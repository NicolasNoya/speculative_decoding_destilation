...
class CustomLoss(nn.Module):
    ...
    def compute_masked_tokens(self, student_logits, tutor_logits, targets):
        ...
        def pad_and_stack(sequences, max_len_logits, max_len_targets, is_logits=False):
            ...
            for seq in sequences:
                if is_logits:
                    ...
                    padding = torch.zeros(
                        pad_len, seq.shape[1], device=seq.device, dtype=seq.dtype
                    )
                else:
        ...
        matches = (windows == code_marker_tokens.unsqueeze(0).unsqueeze(0)).all(
            dim=2
        )  # [batch, seq_len - marker_len + 1]
        ...
        targets_tensor = pad_and_stack(
            truncated_targets, max_len_logits, max_len_targets, is_logits=False
        )
        student_logits_tensor = pad_and_stack(
            truncated_student_logits, max_len_logits, max_len_targets, is_logits=True
        )
        tutor_logits_tensor = pad_and_stack(
